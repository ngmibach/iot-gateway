...
# Secrets to push to the admin/actions repository (used by update_key.yaml workflow etc.)
CA_CN="${CA_CN:-My IOT CA Root}"
CA_PASSWORD="${CA_PASSWORD:-admin}"
COUNTRY="${COUNTRY:-VN}"
STATE="${STATE:-Hanoi}"
CITY="${CITY:-Hanoi}"
ORGANIZATION="${ORGANIZATION:-My IOT Org}"
ORG_UNIT="${ORG_UNIT:-IOT Devices}"
SERVER_CN="${SERVER_CN:-172.20.10.2}"
...
set_repo_secrets() {
  echo "[gitea-seed] Setting repository secrets in ${ADMIN_USER}/${REPO_NAME} (for use by workflows such as update_key.yaml)..."

  for secret_name in CA_CN CA_PASSWORD COUNTRY STATE CITY ORGANIZATION ORG_UNIT SERVER_CN DURATION; do
    local value
    case "$secret_name" in
      CA_CN)        value="$CA_CN" ;;
      CA_PASSWORD)  value="$CA_PASSWORD" ;;
      COUNTRY)      value="$COUNTRY" ;;
      STATE)        value="$STATE" ;;
      CITY)         value="$CITY" ;;
      ORGANIZATION) value="$ORGANIZATION" ;;
      ORG_UNIT)     value="$ORG_UNIT" ;;
      SERVER_CN)    value="$SERVER_CN" ;;
      DURATION)     value="$DURATION" ;;
      *) continue ;;
    esac

    echo "[gitea-seed]   Setting secret ${secret_name}..."

    # Use jq (available in image) to safely build JSON payload (handles any special chars in values)
    local payload
    payload=$(jq -nc --arg v "$value" '{data: $v}' 2>/dev/null || echo "{\"data\":\"$value\"}")

    local resp
    resp=$(curl -sS -w "\n%{http_code}" -o /tmp/secret_${secret_name}.json \
      -u "$ADMIN_USER:$ADMIN_PASS" \
      -X PUT \
      -H "Content-Type: application/json" \
      -H "Accept: application/json" \
      -d "$payload" \
      "$GITEA_URL/api/v1/repos/${ADMIN_USER}/${REPO_NAME}/actions/secrets/${secret_name}" 2>&1 || echo "000")

    local http_code
    http_code=$(echo "$resp" | tail -n1)
    if [ "$http_code" = "201" ] || [ "$http_code" = "204" ]; then
      echo "[gitea-seed]     Secret ${secret_name} set/updated (HTTP $http_code)."
    else
      echo "[gitea-seed]     WARNING: Failed to set secret ${secret_name} (HTTP $http_code)"
      cat /tmp/secret_${secret_name}.json 2>/dev/null || echo "$resp"
    fi
  done

  echo "[gitea-seed] Finished setting repository secrets."
}
